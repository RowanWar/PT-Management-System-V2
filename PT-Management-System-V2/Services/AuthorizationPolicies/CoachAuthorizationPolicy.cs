...
public class CoachAuthorizationHandler : AuthorizationHandler<CoachAuthorizationPolicy> 
{
    ...
    public CoachAuthorizationHandler(ApplicationDbContext context, IHttpContextAccessor httpContextAccessor)
    {
        _context = context;
    ...
    protected override async Task HandleRequirementAsync(AuthorizationHandlerContext context, CoachAuthorizationPolicy requirement)
    {
        ...
        var isMatch = await _context.CoachClients
            .AnyAsync(coacli => coacli.CoachId == coach.CoachId && coacli.ClientId == int.Parse(clientId));
